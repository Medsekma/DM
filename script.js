...
let lang='fr';
const text={
 fr:{placeholder:'Posez votre question…',welcome:'Bonjour. Comment puis-je vous aider concernant nos produits ou votre demande commerciale ?',online:'connecté',offline:'IA non configurée',thinking:'Analyse en cours',aiError:'L’assistant IA ne peut pas répondre actuellement.',missing_key:'La clé Gemini n’est pas détectée sur Render. Vérifiez la variable GEMINI_API_KEY puis redéployez.',invalid_key:'Gemini a refusé la clé API. Vérifiez qu’elle est valide et autorisée dans Google AI Studio.',free_quota:'Le quota gratuit Gemini est temporairement épuisé. Réessayez plus tard.',timeout:'Gemini met trop de temps à répondre. Réessayez.',provider_error:'Gemini a renvoyé une réponse vide ou bloquée. Reformulez la question et réessayez.',rate_limit:'Trop de messages. Attendez une minute.',startQuote:'Démarrer la demande',openForm:'Ouvrir le formulaire',emailUs:'Envoyer un e-mail',products:'Quels produits proposez-vous ?',quality:'Parler à l’équipe commerciale',quote:'Je souhaite un devis',leadProduct:'Très bien. Quel produit vous intéresse ?',leadRole:'Quel est votre profil professionnel ?',leadCountry:'Quel est le pays ou marché de destination ?',leadVolume:'Quel volume annuel approximatif prévoyez-vous ?',leadTimeline:'Quel est votre calendrier souhaité ?',leadCompany:'Quel est le nom de votre organisation ?',leadName:'Quel est votre nom complet ?',leadEmail:'Quelle est votre adresse e-mail professionnelle ? Vous pouvez ajouter votre téléphone après une virgule (facultatif).',leadPhone:'Quel est votre numéro de téléphone ? Vous pouvez aussi choisir Ignorer.',badEmail:'Cette adresse ne semble pas valide. Pouvez-vous la vérifier ?',leadConsent:'Autorisez-vous DM PHARMA à conserver ces informations professionnelles et à vous contacter ?',leadDone:'Merci {name}. Votre demande a été envoyée. Un e-mail de confirmation vous a été adressé.',leadSaved:'Merci {name}. Votre demande a été enregistrée. Notre équipe vous répondra prochainement.',leadLocal:'Le serveur n’a pas pu enregistrer la demande. Utilisez le formulaire de contact ou écrivez à direction@dmpharma.com.tn.',decline:'Compris. Vos coordonnées ne seront pas enregistrées.',sendOk:'Merci. Votre demande a été envoyée et un e-mail de confirmation vous a été adressé.',sendSaved:'Merci. Votre demande a été enregistrée. Notre équipe vous répondra prochainement.',sendFail:'Impossible d’enregistrer la demande. Écrivez à direction@dmpharma.com.tn.',chips:{product:['3 ml','5 ml','Seringue à insuline — 0.5 ml / 1 ml','Plusieurs formats'],role:['Distributeur','Établissement de santé','Achats','Partenaire industriel'],timeline:['Dès que possible','3–6 mois','6–12 mois'],yes:['Oui, j’accepte','Non merci'],skip:['Ignorer']}},
...
function pageMotion(){const root=document.documentElement,y=scrollY;header.classList.toggle('scrolled',y>35);$('.scroll-progress').style.width=`${Math.min(100,y/(root.scrollHeight-innerHeight)*100)}%`;if(!matchMedia('(prefers-reduced-motion: reduce)').matches){$$('.product-visual img,.machine-photo img').forEach(el=>{const r=el.parentElement.getBoundingClientRect();if(r.bottom>0&&r.top<innerHeight)el.style.setProperty('--parallax',`${Math.max(-24,Math.min(24,(innerHeight/2-(r.top+r.height/2))*.045))}px`)})}}
addEventListener('scroll',pageMotion,{passive:true});pageMotion();
menu.addEventListener('click',()=>{const open=nav.classList.toggle('open');menu.setAttribute('aria-expanded',String(open))});$$('nav a').forEach(a=>a.addEventListener('click',()=>{nav.classList.remove('open');menu.setAttribute('aria-expanded','false')}));
...
function resetWelcome(){if(!body)return;body.innerHTML='';bubble(text[lang].welcome);chips([text[lang].products,text[lang].quality,text[lang].quote])}
...
$('#leadForm').addEventListener('submit',async event=>{event.preventDefault();const form=event.currentTarget,message=$('.form-msg',form),button=$('button[type=submit]',form),data=Object.fromEntries(new FormData(form));data.consent=data.consent==='on';data.source='website-form';data.language=lang;button.disabled=true;try{const response=await fetch('/api/leads',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)});const result=await response.json().catch(()=>({}));if(!response.ok)throw new Error();message.className='form-msg';message.textContent=result.emailed?text[lang].sendOk:text[lang].sendSaved;form.reset()}catch{message.className='form-msg error';message.textContent=text[lang].sendFail}finally{button.disabled=false}});
setLang(new URLSearchParams(location.search).get('lang')||localStorage.getItem('dm_lang')||'fr');
if('serviceWorker' in navigator&&location.protocol!=='file:')addEventListener('load',async()=>{const registration=await navigator.serviceWorker.register('/service-worker.js',{updateViaCache:'none'}).catch(()=>null);registration?.update()});
